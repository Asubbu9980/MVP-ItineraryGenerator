...
const destinationList = famousTouristCitiesInIndia.filter(
  (city) => city !== "Hyderabad"
);
...
const IndexPage = () => {
  ...
  const initialValues = {
    ...
    datees: [
      {
        startDate: moment(tomorrow).format("DD MMMM YYYY"),
        endDate: moment(new Date(nextReqDay)).format("DD MMMM YYYY"),
      },
    ],
  };

  const ValidateCity = async (errors, destination, source) => {
    try {
      let isValidDestination = await axios(
        `https://api.openweathermap.org/geo/1.0/direct?q=${destination}&limit=1&appid=258730fe6c89445030503ee5885791ef`
      );
      //    console.log(isValidDestination , 'isValid DEst')
      if (!isValidDestination.length) {
        // console.log('invalid destination')
        errors.destination = "Valid Destination is required";
      }
      let isValidSource = await axios(
        `https://api.openweathermap.org/geo/1.0/direct?q=${source}&limit=1&appid=258730fe6c89445030503ee5885791ef`
      );
      //    console.log(isValidSource , 'isValidSource')
      if (!isValidSource.length) {
        errors.source = "Valid Origin is required";
      }
      return errors;
    } catch (error) {
  ...
  const validate = async (values) => {
    ...
    //     let errorss=  await ValidateCity(errors ,values.destination , values.source )
    //     return {...errors , ...errorss}
    // }
    return errors;
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const formattedStartDate = dayjs(values.datees[0].startDate).format(
        "DD MMMM, YYYY"
      );
      const formattedEndDate = dayjs(values.datees[0].endDate).format(
        "DD MMMM, YYYY"
      );
      ...
      setTripData(null);
      let openAiResponse = await Extract_Info_from_Ai(
        `make a trip from ${values.source} to ${values.destination} from ${values.start_date} to ${values.end_date} `
      );
      ...
      if (source == null || source_close_match?.length) {
      ...
      if (
        source &&
        source_close_match?.length == 0 &&
        destination &&
        destination_close_match?.length == 0
      ) {
        // // console.log("515 is Executing......")
        // console.log(source , source_close_match?.length==0, "source && source_close_match?.length==0")
        // console.log(destination , destination_close_match?.length==0  , "destination && destination_close_match?.length==0")
        // formik.setFieldValue('source', source);
  ...
  //               model: 'gpt-3.5-turbo',
  //               messages: [
  //                 { "role": "system", "content": `Hi you will be acting as AI chatbot, based on the user input send the information in object format exactly  like {startDate:"", endDate:"", destination:"", source:""} , if only one city found make it as destination, and source as Hyderabad if no dates found then assume from tommorrow to 3 days use DD MMM YYYY format` },
...
`,
    ...
    try {
      getTripDetailsApi(payload)
        .then((r) => {
          ...
          setTripTitle(
            `${payload?.source || formik.values.source}  to  ${
              payload?.destination || formik.values.destination
  ...
  const onVoiceSearchTripPlan = async (text) => {
    setSearchText("");
    setTripData(null);
    // const splitedText = text.split(" ");

    let openAiResponse = await Extract_Info_from_Ai(text);
    setAiExtractedInfo(openAiResponse);
    ...
    //     let isValidDestination = await axios(`https://api.openweathermap.org/geo/1.0/direct?q=${v}&limit=1&appid=258730fe6c89445030503ee5885791ef`)
    //     console.log(isValidDestination , 'isvalid')
    //     if (indexed >= 0 || isValidDestination.length>0) {
    //         destination = v
    //     }
    // }
    // getResult(openAiResponse)
    ...
    //let Destination=openAiResponse?.destination || openAiResponse["destination"]

    // console.log(destination , 'destination')

    // console.log(openAiResponse , "openAiResponseopenAiResponse")

    if (destination && source) {
      //formik.setFieldValue('destination', destination.charAt(0).toUpperCase() + destination.slice(1).toLowerCase())
      setTripTitle(
        `${source}  to  ${destination}  from  ${start_date}  to  ${
      ...
      // getTripByVoice(text , openAiResponse?.destination)
      if (
        source &&
        source_close_match?.length == 0 &&
        destination &&
  ...
  const onChangeDestinationList = (value) => {
    setTopTouristCities({
      ...topTouristCities,
      destinationsList: famousTouristCitiesInIndia.filter(
        (city) => city !== value
      ),
    });
  };
  const onChangeSourcesList = (value) => {
    setTopTouristCities({
      ...topTouristCities,
      originsList: famousTouristCitiesInIndia.filter((city) => city !== value),
    });
  };
  ...
  const handleSelectedSource = (city) => {
    formik.setFieldValue("source", city);
    console.log("handleSelectedSource", city);
  };
  ...
  const handleSelect = (ranges) => {
    ...
    if (diffInDays > 6) {
      ...
      formik.setFieldValue(
        "start_date",
        moment(restrictedRange?.startDate).format("DD MMMM, YYYY")
      );
      formik.setFieldValue(
        "end_date",
        moment(restrictedRange?.endDate).format("DD MMMM, YYYY")
      );
      formik.setFieldValue("datees", [restrictedRange]);
    } else {
      formik.setFieldValue(
        ...
        <div className="searchBanner">
          <Container className="banner-container">
            <div className="col-12">
              <h5 className="banner-title">
                Unlock the Power of Personalized Travel with Ai
              </h5>
            </div>
            <div className="col-12 mb-3">
              <div className="search">
                <div className="input-bar">
                  ...
                  <input
                    type="search"
                    placeholder="Speak/Search for your vacation trip"
                    value={searchText}
                ...
                {searchText && isValidDestination ? (
                  <div>
                    <p
                      className="text-danger fw-bold my-1 ms-3"
                      //  style={{ backgroundColor: '#ffffff', maxWidth: '320px', borderRadius: '8px' }}
                    >
                      Please Try with another destination ...
                    </p>
                  </div>
                ) : (
                  ""
            ...
            >
              ...
              >
                      ...
                      <div className="row form-sub-row-container-cls">
                        ...
                        <div className="col col-12 col-sm-6 col-md-6 col-lg-3 px-2 px-sm-0">
                          ...
                          <Autocomplete
                            ...
                            onInputChange={(e, inputValue) => {
                              formik.setFieldValue("source", inputValue);
                            }}
                            renderInput={(params) => {
                        ...
                        <div className="col col-12 col-sm-6 col-md-6 col-lg-3 px-2 px-sm-0">
                          ...
                          <Autocomplete
                            ...
                            popupIcon={<ExpandMoreIcon />}
                            onInputChange={(e, inputValue) => {
                              formik.setFieldValue("destination", inputValue);
                            }}
                            onChange={(e, value) => {
                      ...
                      <div className="row">
                        <div className="col col-12 d-flex justify-content-center align-items-center">
                          <Button
                            ...
                            style={{}}
                            startIcon={<Icon icon="cuida:search-outline" />}
                          >
                            {!loaderContext.loading
                              ? "Start Planning"
                              : "Planning..."}
                ...
                <div
                  ...
                  className="home-navigation-modes-container"
                >
                  {/* <TransportModes></TransportModes> */}
                  <Link to="/chatbot">
                    <button className="navigation-btn-cls chatbot-btn me-3">
                      Chatboat AI {"  "}{" "}
                      <Icon
                  ...
                  <Link to="/preferential-search">
                    <button className="navigation-btn-cls custom-search">
                      Customized Search {"  "}
                      <Icon
                        icon="teenyicons:top-right-outline"
                        className="ms-2"
...

          {
            <SpeechRecognitionModal
              ...
              onClose={onClose}
              onVoiceSearchTripPlan={onVoiceSearchTripPlan}
          ...
          {showSuggestedCities && (
            <SuggestedCities
              ...
              showSuggestedCities={showSuggestedCities}
              setShowSuggestedCities={setShowSuggestedCities}
            />
          )}
        </div>
        <>
          {!loaderContext.loading ? (
            <div
              style={{
                ...
                paddingBottom: `${
                  tripData != null &&
                  tripData?.places_visited &&
                  tripData?.places_visited.length > 0
                    ? "8px"
                    : "32px"
                }`,
                paddingTop: "20px",
            ...
            >
              ...
              tripData?.places_visited &&
              tripData?.places_visited.length > 0 &&
              isKeyInArray(tripData?.places_visited, "description") ? (
                ...
                tripData?.places_visited.length > 0 && (
                  <div>
                    <div className="search_info">
                      ...
                      <h5>
                        No Trip Data available <br />
                        Please check for other popular locations.
                      </h5>
                    </div>
                  </div>
                )
              ...
              {tripData === null && (
                <div>
                  <div className="search_info">
                    <img src={searchIocn} alt="logo" />
                    <h5>
                      Click the Top Button start your <br />
                      vacation planning.
                    </h5>
                  </div>
                </div>
